# Search API - Java Example

This example demonstrates how to use the Gloo AI Search API with Java to perform semantic search on your ingested content.
...
```bash
mvn exec:java -Dexec.args='search "How can I know my purpose?"'
```

With custom result limit:
```bash
mvn exec:java -Dexec.args='search "purpose" 5'
```
...
```bash
mvn exec:java -Dexec.args='filter "purpose" "Article,Video" 10'
```
...
```bash
mvn exec:java -Dexec.args='rag "How can I know my purpose?" 3'
```
...
```bash
mvn exec:java -Dexec.args="server 8080"
```
