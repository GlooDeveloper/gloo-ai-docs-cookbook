# Search API - Go Example

This example demonstrates how to use the Gloo AI Search API with Go to perform semantic search on your ingested content.
...
```bash
go run . search "How can I know my purpose?"
```

With custom result limit:
```bash
go run . search "purpose" 5
```
...
```bash
go run . filter "purpose" "Article,Video" 10
```
...
```bash
go run . rag "How can I know my purpose?" 3
```
