 ...
 * </ul>
 *
 * <p>Usage: mvn -q compile exec:java -Dexec.mainClass=com.gloo.streaming.tests.Step3SseParsingTest
 */
public class Step3SseParsingTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            try {
                StreamClient.makeStreamingRequest("Hello", "invalid-token-xyz");
                throw new RuntimeException("Expected makeStreamingRequest to throw with bad credentials");
            } catch (RuntimeException e) {
                if (e.getMessage().startsWith("Expected makeStreamingRequest")) throw e;
                String msg = e.getMessage().toLowerCase();
                if (e.getMessage().contains("401") || e.getMessage().contains("403")
                        || msg.contains("auth") || msg.contains("token")
                        || msg.contains("permission") || msg.contains("invalid")) {
                    System.out.println("✓ Bad credentials caught (pre-stream): " + e.getMessage());
                } else {
            ...
            System.out.println("   Next: Token Extraction & Accumulation\n");
