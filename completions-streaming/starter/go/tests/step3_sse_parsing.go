...
//
// Usage: go run tests/step3_sse_parsing.go
...
func main() {
	...
	_, err = streaming.MakeStreamingRequest("Hello", "invalid-token-xyz")
	if err == nil {
	...
	if strings.Contains(msg, "401") || strings.Contains(msg, "403") ||
		strings.Contains(msgLower, "auth") || strings.Contains(msgLower, "token") ||
		strings.Contains(msgLower, "permission") || strings.Contains(msgLower, "invalid") {
		fmt.Printf("✓ Bad credentials caught (pre-stream): %s\n", msg)
	} else {
		fail(fmt.Sprintf("Expected auth error, got: %s", msg))
	}

	fmt.Println("\n✅ Streaming request and SSE parsing working.")
	fmt.Println("   Next: Token Extraction & Accumulation")
}
