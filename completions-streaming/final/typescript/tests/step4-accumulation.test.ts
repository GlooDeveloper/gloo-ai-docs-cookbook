 ...
 *
 * Usage: node --loader ts-node/esm --no-warnings tests/step3-accumulation.test.ts
 */
...
async function testStep3(): Promise<void> {
  ...
  try {
    ...
    let chunk: SSEChunk = { choices: [{ delta: { content: "Hello" }, finish_reason: null }] };
    ...
    chunk = { choices: [{ delta: { content: undefined }, finish_reason: null }] };
    ...
    console.error("   - streamCompletion: increment tokenCount only when content is non-empty");
