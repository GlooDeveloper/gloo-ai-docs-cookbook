# Upload Files - TypeScript Example

This example demonstrates how to use the Gloo AI Data Engine Files API with TypeScript to upload files directly for processing and AI-powered search.
...
```bash
npx ts-node index.ts batch ../sample_files
```
...
```bash
npx ts-node index.ts meta ../sample_files/developer_happiness.txt --title "Developer Happiness" --author "John Doe" --tags "development,culture"
```
...
Then run with:
```bash
node dist/index.js single ../sample_files/developer_happiness.txt
```
