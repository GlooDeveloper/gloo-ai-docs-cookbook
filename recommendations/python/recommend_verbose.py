...
Usage:
  python recommend_verbose.py <query> [item_count]
...
class VerboseRecommendationsClient:
    ...
    def get_verbose(self, query: str, item_count: int = DEFAULT_ITEM_COUNT) -> list[Any]:
        ...
        try:
            ...
            raise Exception(f"Verbose recommendations request failed: {e}")


class RecommendVerboseApp:
    ...
    def run(self, query: str, item_count: int = DEFAULT_ITEM_COUNT) -> None:
        ...
        try:
            print(f"Fetching recommendations (with previews) for: '{query}'")
            print(f"Requesting up to {item_count} items\n")

            items = self.client.get_verbose(query, item_count)
            ...
            for i, item in enumerate(items, 1):
                ...
                if uuids:
                    ...
                    if snippet:
                        preview = snippet[:SNIPPET_PREVIEW_CHARS]
                        ellipsis = "..." if len(snippet) > SNIPPET_PREVIEW_CHARS else ""
    ...
    def print_usage(self) -> None:
        print("Usage:")
        print("  python recommend_verbose.py <query> [item_count]")
        print()
        print("Arguments:")
        ...
        print()
        print("Examples:")
        print('  python recommend_verbose.py "How do I deal with anxiety?"')
        print('  python recommend_verbose.py "parenting teenagers" 3')
